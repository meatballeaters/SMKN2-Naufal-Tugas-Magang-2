...
class Records {
	...
	public function listRecords(){
		...
		while ($record = $result->fetch_assoc()) {
			...
			$rows[] = $record['bayar'];
			$rows[] = $record['status'];
			$rows[] = '<button type="button" name="view" id="'.$record["id"].'" class="btn btn-outline-primary text-light btn-xs view" >View</button>';
			$rows[] = '<button type="button" name="update" id="'.$record["id"].'" class="btn btn-outline-warning text-light btn-xs update">Update</button>';
			$rows[] = '<button type="button" name="delete" id="'.$record["id"].'" class="btn btn-outline-danger text-light btn-xs delete" >Delete</button>';
			$records[] = $rows;
		}
	...
	public function updateRecord(){
		...
		if($this->id) {
			...
			SET name= ?, bayar = ?, pesanan = ?, status = ?, jam = ?
			WHERE id = ?");
	...
	public function addRecord(){
		...
		if($this->name) {
			...
			$stmt = $this->conn->prepare("
			INSERT INTO ".$this->recordsTable."(`name`, `bayar`, `pesanan`, `status`, `jam`)
			VALUES(?,?,?,?,?)");
